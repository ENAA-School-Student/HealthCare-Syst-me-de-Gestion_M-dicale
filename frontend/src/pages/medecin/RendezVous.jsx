                    ...
                    <tr key={rdv.id}>
                      ...
                      <td>
                        <div className="td-actions">
                          <button className="btn btn-ghost btn-sm" onClick={() => setSelected(rdv)}><Eye size={14} /> Détails</button>
                          {rdv.statut === 'EN_ATTENTE' && (
                            <>
                              <button className="btn btn-primary btn-sm" onClick={() => handleConfirm(rdv.id)}><CheckCircle size={14} /> Confirmer</button>
                              <button className="btn btn-danger btn-sm" onClick={() => handleCancel(rdv.id)}><XCircle size={14} /> Annuler</button>
                            </>
                          )}
                          {rdv.statut === 'CONFIRME' && (
                            <button className="btn btn-danger btn-sm" onClick={() => handleCancel(rdv.id)}><XCircle size={14} /> Annuler</button>
        ...
        {selected && (
          <div style={{ padding: '0 0 8px' }}>
            <div className="form-group">
              ...
              <p>{selected.patientNom || selected.patient?.nom || '—'}</p>
            </div>
            <div className="form-group">
              <label className="form-label">Date</label>
              <p>{selected.date ? new Date(selected.date).toLocaleDateString('fr-FR') : '—'}</p>
            </div>
            <div className="form-group">
