...
export default function Patients() {
  ...
  if (loading && patients.length === 0) return <div className="loading-container"><div className="spinner" /></div>;
    ...
    <div>
      ...
      <div className="card">
        <div className="card-header">
          ...
          <button className="btn btn-primary" onClick={openCreate}><Plus size={16} /> Ajouter Patient</button>
        </div>
        {patients.length === 0 ? (
              ...
              <tbody>
                {patients.map((p) => (
                  <tr key={p.id}>
                    ...
                    <td>{p.prenom}</td>
                    <td>{p.telephone}</td>
                    <td>{p.dateNaissance ? new Date(p.dateNaissance).toLocaleDateString('fr-FR') : '—'}</td>
                    <td>
                      <div className="flex gap-2">
                        <button className="btn btn-ghost btn-sm" onClick={() => openEdit(p)} title="Modifier"><Pencil size={14} /></button>
                        <button className="btn btn-danger btn-sm" onClick={() => handleDelete(p.id)} title="Supprimer"><Trash2 size={14} /></button>
                        <button className="btn btn-secondary btn-sm" onClick={() => handleDownload(p.id)} title="Télécharger rapport"><Download size={14} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      >
        <form id="patient-form" onSubmit={handleSubmit} style={{ padding: '1rem' }}>
